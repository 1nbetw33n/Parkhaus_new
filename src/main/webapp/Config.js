        ...
        Instance: function () {
            ...
            this.start = async () => {
                    ...
                    add_image( more ){
                        ...
                        if ( more > 0 ){
                            ...
                            for ( let i = this._max; i > this._max + more; i-- ){
                                ...
                                if ( space ){
                                    garage.removeCar( garage._space_index[ space.id ] );
                ...
                class Car {
                    ...
                    hash(){
                        return self.hash ? self.hash.md5( self.SALT + this.nr + this.begin() ) : this.nr;
                    }
                    inGarage(){
                    ...
                    toArray(){
                        return [ this.nr, this.timer, this.duration(), this.price(), this.hash(), this.color(), this.space, this.client_category, this.vehicle_type, this.license ].map(x => (x || '_').toString() );
                    }
                    tooltip(){
                ...
                if ( config_string ){
                    ...
                    Object.keys( header ).forEach( className => {
                        ...
                        if ( className === 'max' ) garage.max = config.max;
                        garage.rerender();
                    });
                ...
                function oninput( e ) {
                    ...
                    switch( this.classList.toString() ){
                        case "max":
                            garage.add_image( parseInt( this.value ) - garage.max );
                            garage.max = parseInt( this.value );
                            garage.rerender();
                            break;
                ...
                function randomLicense(){
                    if ( self.license_max ){
                        if ( typeof self.license_max === 'string' ) self.license_max = parseInt( self.license_max );
                        return 1 + getRandomInt( self.license_max );
                    } else {
                        return 0;
                    }
                }
